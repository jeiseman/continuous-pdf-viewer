...
function cpdfv_frontend_assets() {
    ...
    wp_localize_script( 'cpdfv-core-js', 'cpdfvSettings', array(
        ...
        'standardFontUrl' => plugins_url( 'lib/pdfjs/standard_fonts/', __FILE__ ),
        'cMapUrl'         => plugins_url( 'lib/pdfjs/cmaps/', __FILE__ )
    ));
}
...
function cpdfv_admin_assets( $hook ) {
    ...
    wp_enqueue_style( 'cpdfv-admin-css', plugins_url( 'pdf-viewer-admin.css', __FILE__ ), array(), '2.1.14' );
    wp_enqueue_style( 'wp-color-picker' );
    wp_enqueue_script( 'wp-color-picker' );
    wp_enqueue_media();
    wp_enqueue_script( 'cpdfv-admin-js', plugins_url( 'pdf-viewer-admin.js', __FILE__ ), array( 'jquery', 'wp-color-picker' ), '2.1.14', true );
}
