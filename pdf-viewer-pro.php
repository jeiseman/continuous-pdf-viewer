...
/**
 * 1. Frontend Enqueue: Loads the core viewer and PDF.js
 */
add_action( 'wp_enqueue_scripts', 'cpv_frontend_assets' );
function cpv_frontend_assets() {
    // Load the new Core Viewer CSS file
    wp_enqueue_style( 'cpv-core-css', plugins_url( 'pdf-viewer-core.css', __FILE__ ), array(), '2.0.0' );
    // Register the external PDF.js library from CDN
...
function cpv_admin_assets( $hook ) {
    ...
    wp_enqueue_style( 'cpv-admin-css', plugins_url( 'pdf-viewer-admin.css', __FILE__ ) );
...
?>
<div id="cpv-wrap">
<div class="cpv-hero"><span class="cpv-hero-badge">Shortcode Generator</span><h1>PDF Viewer</h1><p>Configure your viewer below &mdash; shortcode updates live on the right. Copy and paste it anywhere.</p></div>
<div class="cpv-grid">
...
<div class="cpv-card"><div class="cpv-card-head"><div class="cpv-card-icon" style="background:#e8faf0;color:#059669">&#9881;&#65039;</div><h2>Feature Visibility</h2></div>
<div class="cpv-card-body">
...
<div class="cpv-card-body">
  ...
  <div class="cpv-row">
    <div class="cpv-field"><label>Cover Button Text</label><input type="text" data-attr="cover_button_text" data-default="Open PDF" value="Open PDF"></div>
    <div class="cpv-field"><label>Cover Height</label><input type="text" data-attr="cover_height" data-default="720px" value="720px"></div>
  </div>
  <div class="cpv-row">
    <div class="cpv-field"><label>Cover Hint</label><input type="text" data-attr="cover_hint" data-default="Click to open the document viewer" value="Click to open the document viewer"></div>
    <div class="cpv-field"><label>Show Hover Overlay</label><select data-attr="cover_overlay" data-default="no"><option value="no">No</option><option value="yes">Yes</option></select></div>
  </div>
  <div class="cpv-row-3">
    <div class="cpv-field"><label>Cover Button Align</label><select data-attr="cover_button_align" data-default="left"><option value="left">Left</option><option value="center">Centre</option><option value="right">Right</option><option value="stretch">Stretch</option></select></div>
    <div class="cpv-field"><label>Cover Button Padding</label><input type="text" data-attr="cover_button_padding" data-default="12px 18px" value="12px 18px"></div>
    <div class="cpv-field"><label>Cover Button Radius</label><input type="text" data-attr="cover_button_radius" data-default="10px" value="10px"></div>
  </div>
  <div class="cpv-row">
    <div class="cpv-field"><label>Overlay Text</label><input type="text" data-attr="cover_overlay_text" data-default="Click to Open" value="Click to Open"></div>
    <div class="cpv-field"><label>Overlay Background</label><input type="text" data-attr="cover_overlay_bg" data-default="rgba(0,0,0,0.45)" value="rgba(0,0,0,0.45)"></div>
  </div>
  <div class="cpv-row-3">
    <div class="cpv-field"><label>Overlay Text Colour</label><input type="text" data-attr="cover_overlay_color" data-default="#ffffff" value="#ffffff"></div>
    <div class="cpv-field"><label>Overlay Padding</label><input type="text" data-attr="cover_overlay_padding" data-default="18px 24px" value="18px 24px"></div>
    <div class="cpv-field"><label>Overlay Radius</label><input type="text" data-attr="cover_overlay_radius" data-default="0px" value="0px"></div>
  </div>
  <div class="cpv-row-3">
    <div class="cpv-field"><label>Overlay Font Size</label><input type="text" data-attr="cover_overlay_font_size" data-default="18px" value="18px"></div>
    <div class="cpv-field"><label>Overlay Font Weight</label><input type="text" data-attr="cover_overlay_font_weight" data-default="700" value="700"></div>
    <div class="cpv-field"></div>
  </div>
  <div class="cpv-row">
    <div class="cpv-field"><label>Viewer Shadow</label><select data-attr="viewer_shadow" data-default="yes"><option value="yes">Show</option><option value="no">Remove</option></select></div>
    <div class="cpv-field"></div>
  </div>
  <div class="cpv-field"><label>Loading Text</label><input type="text" data-attr="loading_text" data-default="Loading document..." value="Loading document..."></div>
  <div class="cpv-field"><label>Error Text</label><input type="text" data-attr="error_text" data-default="Could not load the document. Please check the file URL." value="Could not load the document. Please check the file URL."></div>
</div></div>

</div>

<div class="cpv-output-col"><div class="cpv-output-card">
<div class="cpv-output-head"><h3>&#128203; Generated Shortcode</h3><div style="display:flex;align-items:center;gap:8px"><span style="font-size:11px;color:rgba(255,255,255,0.35)">Live</span><div class="cpv-live-dot"></div></div></div>
<div class="cpv-output-body">
  <textarea id="cpv-shortcode-output" readonly></textarea>
  <button type="button" class="cpv-copy-btn" id="cpv-copy-btn"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg> Copy Shortcode</button>
  <div class="cpv-attr-count" id="cpv-attr-count"></div>
  <div style="text-align:center"><button type="button" class="cpv-reset-btn" id="cpv-reset-btn">&#8634; Reset to Defaults</button></div>
  <div class="cpv-import-card">
    <label class="cpv-import-label" for="cpv-import-shortcode">Import existing shortcode</label>
    <textarea id="cpv-import-shortcode" placeholder='Paste a full shortcode like [pdf_viewer url="..." cover_mode="yes"]'></textarea>
    <div class="cpv-import-actions">
      <button type="button" class="cpv-import-btn" id="cpv-import-btn">Import Shortcode</button>
      <button type="button" class="cpv-clear-import-btn" id="cpv-clear-import-btn">Clear Import</button>
    </div>
    <div id="cpv-import-feedback"></div>
  </div>
</div></div></div>
...
/* =================================================================
 *  PART 2 - SHORTCODE RENDERER
 * ================================================================= */

add_filter( 'the_content', 'cpv_protect_shortcode', 9 );
function cpv_protect_shortcode( $content ) {
    return preg_replace_callback('/\[pdf_viewer\b[^\]]*\]/',function($m){return str_replace(array("\n","\r","<br />","<br>","<p>","</p>"),' ',$m[0]);},$content);
}
...
function cpv_register_pdf_block() {
    register_block_type( plugin_dir_path( __FILE__ ) . 'block', array(
        'api_version'     => 2,
        'editor_script'   => 'cpv-block-editor-script', // Your enqueued index.js
        'render_callback' => 'cpv_render_block'         // THIS IS THE CRITICAL LINE
    ) );
}
add_action( 'init', 'cpv_register_pdf_block' );

function cpv_render_block( $attributes ) {
    $mapped_atts = array();
    
    // Map Gutenberg camelCase back to Shortcode snake_case
    $keys = array(
        'url', 'height', 'tablet_height', 'mobile_height', 'width', 'max_width', 'border_radius',
        'accent', 'bg_color', 'surface_color', 'text_color', 'font', 'font_display',
        'download_desktop', 'download_tablet', 'download_mobile',
        'print_desktop', 'print_tablet', 'print_mobile',
        'fullscreen_desktop', 'fullscreen_tablet', 'fullscreen_mobile',
        'search_desktop', 'search_tablet', 'search_mobile',
        'thumbnails_desktop', 'thumbnails_tablet', 'thumbnails_mobile',
        'zoom_desktop', 'zoom_tablet', 'zoom_mobile',
        'theme_toggle_desktop', 'theme_toggle_tablet', 'theme_toggle_mobile',
        'page_nav_desktop', 'page_nav_tablet', 'page_nav_mobile',
        'fit_width_btn_desktop', 'fit_width_btn_tablet', 'fit_width_btn_mobile',
        'fit_height_btn_desktop', 'fit_height_btn_tablet', 'fit_height_btn_mobile',
        'status_bar_desktop', 'status_bar_tablet', 'status_bar_mobile',
        'keyboard', 'default_zoom', 'min_zoom', 'max_zoom', 'zoom_step', 'start_page', 'sidebar_open',
        'cover_mode', 'cover_button', 'cover_button_text', 'cover_height', 'cover_hint', 'cover_overlay',
        'cover_button_align', 'cover_button_padding', 'cover_button_radius',
        'cover_overlay_text', 'cover_overlay_bg', 'cover_overlay_color',
        'cover_overlay_padding', 'cover_overlay_radius', 'cover_overlay_font_size', 'cover_overlay_font_weight',
        'viewer_shadow', 'loading_text', 'error_text'
    );

    foreach($keys as $key) {
        // Convert snake_case to camelCase to check if it exists in the block attributes
        $camelKey = lcfirst(str_replace(' ', '', ucwords(str_replace('_', ' ', $key))));
        if (isset($attributes[$camelKey])) {
            $mapped_atts[$key] = $attributes[$camelKey];
        }
    }

    return cpv_render_shortcode( $mapped_atts );
}
